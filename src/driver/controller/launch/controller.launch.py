...
def launch_setup(context):
    ...
    arm_kinematics_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(arm_kinematics_package_path, 'launch/arm_kinematics_node.launch.py')),
    )
    move_controller_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(controller_package_path, 'launch/move_controller.launch.py')
        ]),
        launch_arguments={
            'base_frame': base_frame,
        }.items()
