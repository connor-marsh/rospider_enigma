...
- [ ] **`lr`**: still 2e-3. Batch went up 4x, so sqrt-scaling suggests trying 4e-3. **Open — see bite #10.**
- [ ] **`chunks_per_epoch`**: set so one epoch takes ~5–30 s wall-clock. **Blocked on bite #3** — changing it currently perturbs the LR schedule.
...
| 3 | ❌ | **Changing `chunks_per_epoch` silently changes the LR schedule** while `sched.step()` is per-epoch. | **Open.** LR decay is measured in epochs, not gradient steps, so raising `chunks_per_epoch` makes the LR effectively hotter per unit of progress. Fix: `T_max = epochs * chunks_per_epoch` and step per chunk. Blocks the `chunks_per_epoch` tuning item above. |
...
| 11 | ❌ | **`clip_grad_norm_(1.0)` can mask a too-large LR.** Stability under clipping is not evidence the LR is well-chosen. | **Open, never investigated.** Worth logging the *pre-clip* gradient norm for a few hundred steps: if it's routinely ≫ 1.0, clipping is truncating most updates and the LR is too hot. |
...
| 13 | ❓ | **Re-run the ONNX parity check after changing `spike_fn`.** Confirm diff < 1e-4. | Check is present in `export_onnx` and prints `PyTorch vs ONNX max diff`. **Needs confirming on the next export** — the signature changed again with the recurrence removal (5 state tensors → 3). |
