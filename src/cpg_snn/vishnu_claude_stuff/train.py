...
class FastSigmoidSpike(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        (x,) = ctx.saved_tensors
        return grad_out / (ctx.slope * x.abs() + 1.0) ** 2, None
...
def export_onnx(model, out_dir, device, cfg):
    ...
    dummy = (torch.zeros(1, 4, device=device),
             ...
             *[torch.zeros(1, model.G, model.Hg, device=device) for _ in range(5)])
    ...
    try:
        ...
        diff = max(float(np.abs(a - b).max()) for a, b in zip(pt_out, ort_out))
